OakRatings:
; if you caught at most this many, play this sound, load this text
; (text is defined in engine/events/prof_oaks_pc.asm)
	dw   9, SFX_DEX_FANFARE_1, OakRating01
	dw  19, SFX_DEX_FANFARE_1, OakRating02
	dw  34, SFX_DEX_FANFARE_2, OakRating03
	dw  49, SFX_DEX_FANFARE_2, OakRating04
	dw  64, SFX_DEX_FANFARE_3, OakRating05
	dw  79, SFX_DEX_FANFARE_3, OakRating06
	dw  94, SFX_DEX_FANFARE_4, OakRating07
	dw 109, SFX_DEX_FANFARE_4, OakRating08
	dw 124, SFX_CAUGHT_MON,    OakRating09
	dw 139, SFX_CAUGHT_MON,    OakRating10
	dw 154, SFX_DEX_FANFARE_5, OakRating11
	dw 169, SFX_DEX_FANFARE_5, OakRating12
	dw 184, SFX_DEX_FANFARE_6, OakRating13
	dw 199, SFX_DEX_FANFARE_6, OakRating14
	dw 214, SFX_DEX_FANFARE_7, OakRating15
	dw 229, SFX_DEX_FANFARE_7, OakRating16
	dw 239, SFX_DEX_FANFARE_8, OakRating17
	dw 248, SFX_DEX_FANFARE_8, OakRating18
	dw  -1, SFX_DEX_FANFARE_8, OakRating19
